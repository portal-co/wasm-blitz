...
10. **New AArch64 regalloc-backed backend** — net-new integration (an AArch64 `RegKind`/instruction-selection adapter for `regalloc_frontend`), reusing the already-existing upstream `asm-aarch64` regalloc primitives. Its control-flow model already matches the target, so only the data-flow half is new.
   - **In progress** — `blitz-aarch64::naive.rs` routes consts/locals plus the full I64 **and I32** binop set RV already migrated (`Add`/`Sub`/`Mul`/`And`/`Or`/`Xor`/`Shl`/`ShrU`/`ShrS`; I32 paths zero-extend). x86-64 `regalloc_core` covers add/sub/and/or/xor **plus** two-operand `imul` (`I32Mul`/`I64Mul`). x86 shifts still flush (CL fixed-reg constraint). Loads/stores/compares remain follow-up toward the RV reference set. `lfi.rs` also resets `state.regalloc = None` per function (needed since it shares `naive::State`, even though LFI itself stays CTX-based/pure-stack per step 9's note — this is just keeping the shared struct's per-function invariant, not a data-flow migration of LFI).
