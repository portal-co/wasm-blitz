...
| `blitz-x86-64/src/sysv.rs` | compile-time label stack (own `SysVCtrl`/`ctrl_stack`, CTX-free) | regalloc-backed for const/local/add/sub (`regalloc_core`), delegates the rest to `naive::_handle_op` (flushed first) |
| `blitz-x86-64/src/lfi.rs` | runtime CTX-stack trick — built on `naive::State`, **not** `sysv.rs` (correcting an earlier assumption in this doc; see step 9) | pure stack — zero regalloc usage |
...
RISC-V's `naive.rs` and AArch64's `naive.rs` already agree on the control-flow model (compile-time label stack). x86-64's `sysv.rs` *also* already uses that model (its own `SysVCtrl`, independent of the CTX trick `naive.rs`/`fast.rs`/`lfi.rs` use) and now has a regalloc-backed core for its most common ops, delegating the rest to `naive::_handle_op`'s pure stack machine (flushed first). x86-64's `naive.rs` and `lfi.rs` remain outside this unification — long-term deprecation target (`naive.rs`) / net-new-integration-if-ever-needed (`lfi.rs`) once the regalloc-based path is mature enough.
...
8. **Gave x86-64's `sysv.rs` a regalloc-backed data-flow core** — done (commit `c2dd1c2`, plus an upstream fix to the external `asm-regalloc` crate — see below). Turned out bigger and more delicate than it first looked:
   - **Control flow needed no migration.** `sysv.rs`'s own `SysVCtrl` enum already implements the compile-time label-stack model independently of `naive.rs`, already tested and working. Left as-is (not swapped for `control_flow::Frame`) — the two are shaped alike but merging them wasn't necessary for this increment.
   - **The real work**: added a `regalloc: Option<X64RegAlloc>` field to `SysVState` (reset every `StartFn` — per-function state, unlike the monotonic `label_index`), added `flush()` calls to every one of `sysv.rs`'s own control-flow/branch/call arms (`If`, `Else`, `End`, `Br`, `BrIf`, `BrTable`, `LocalTee`, the 4 `AllStack` call arms) and inside `sysv_emit_epilogue` and the probe emitters (`sysv_emit_control_flow_probe`/`sysv_emit_indexed_probes` — these fire *without* a flush at exactly the points a WASM operand is most likely register-held, and use RAX as an assumed-free scratch register, a leftover assumption from before this integration). New module `blitz-x86-64/src/regalloc_core.rs` (const/`LocalGet`/`LocalSet`/`I32Add`|`I64Add`/`I32Sub`|`I64Sub` via `regalloc_frontend`) plus a `RegAllocW` adapter in `codegen.rs` implementing both `RegAllocWriter` and `ControlFlowWriter`. `init_regalloc` reserves rsp/rbp/**RAX**/r11/r14/r15 — RAX specifically because of the probe-emitter hazard above; without reserving it, the allocator could legitimately hand a WASM operand the exact register probes assume is free.
   - **Found and fixed two real bugs surfaced only by this integration** (both now fixed, not just worked around): (1) `asm-regalloc`'s `flush()` masked its scan index for reads but not writes, causing an out-of-bounds panic once the scan advanced past register 31 — fixed upstream in `/Users/g/Code-local/portal-hot/asm-arch` (separate repo, patched into this workspace; see that repo's own commit). (2) `regalloc_frontend`'s `binop`/`compare`/`store` popped two operands in a row assuming they'd land in different registers, which silently breaks whenever the allocator's `tos` is `None` for both pops (e.g. right after a flush, or right after a naive-delegated instruction pushes an untracked raw value, as happens whenever an FP op — not yet regalloc-covered — is immediately followed by an integer op that is) — both pops would select the *same* register, and the second real pop instruction would clobber the first value. Fixed with `guard_reg`/`unguard_reg` (temporarily reserve the first-popped register so the second pop is forced elsewhere) in `regalloc_frontend.rs` itself — a latent bug in RISC-V's path too, just never triggered there.
   - Verified against the existing real-execution test suite (`assert_native_sysv_const` and the whole `test_unicorn_x86_64_*`/`test_sysv_tracing_*`/`test_native_x86_64_sysv_*` family) — 361/361, no new tests added yet for the newly-regalloc-backed ops specifically (follow-up).
9. **`lfi.rs` does *not* inherit this work — correcting an earlier assumption in this doc.** Read `crates/blitz-x86-64/src/lfi.rs` in full: it's built on `naive::State` (`pub use crate::naive::State;`), reuses `naive.rs`'s CTX-based runtime-stack trick for control flow via its own `emit_start_body`/`emit_local_get`/`emit_local_set` (`BackendAbi` trait methods, all doing raw `xchg RSP,CTX` + push/pop — the *same* pattern as `naive.rs`, not `sysv.rs`'s), and `lfi_instr`'s own doc comment says outright: "all others delegate to the naive implementation." It shares no code path with `sysv_handle_insn` at all. LFI is therefore architecturally adjacent to `naive.rs` (CTX-based, pure-stack) — giving it a regalloc-backed core would be a net-new integration of the same shape as step 10 below, not something that falls out of step 8 for free. Not started; not currently planned — revisit only if there's a concrete need, matching the reasoning that kept `naive.rs` itself out of scope.
10. **New AArch64 regalloc-backed backend** — net-new integration (an AArch64 `RegKind`/instruction-selection adapter for `regalloc_frontend`), reusing the already-existing upstream `asm-aarch64` regalloc primitives. Its control-flow model already matches the target, so only the data-flow half is new.
   - **In progress, not "not started"** — this entry was stale relative to the tree. `blitz-aarch64::naive.rs` (shared by both `naive.rs`'s own direct entry and `sysv.rs`'s AAPCS64 wrapper) got a `regalloc: Option<RegAlloc<asm_aarch64::regalloc::RegKind, 32, Frames<..>>>` field on `State`, reset per `StartFn` (mirroring x86-64's own per-function reset), and `I64Const`/`I32Const`/`LocalGet`/`LocalSet`/`I64Add`/`I64Sub` route through `regalloc_frontend`'s shared dispatch; everything else flushes first via the same `regalloc_covered` gate x86-64 uses. `lfi.rs` also resets `state.regalloc = None` per function (needed since it shares `naive::State`, even though LFI itself stays CTX-based/pure-stack per step 9's note — this is just keeping the shared struct's per-function invariant, not a data-flow migration of LFI).
   - `asm-aarch64/src/regalloc.rs`'s `Cmd::Push`/`Cmd::Pop` stride was fixed from 8 to 16 bytes — AAPCS64 requires SP to stay 16-byte aligned across any call/trap, and this regalloc's own operand-stack pushes share the same SP as callers' own (non-regalloc) 16-byte pushes (e.g. `naive::WASM_SLOT`); mixing 8- and 16-byte strides on one SP would desynchronize the two.
   - **Found and fixed a real, pre-existing, regalloc-independent bug this integration exposed**: `blitz-aarch64::naive.rs`'s `Instruction::If` allocates a separate `else_lbl` (the false-branch target) and `end_lbl`, but only `Instruction::Else`'s handler ever binds `else_lbl` — for an `if` with no matching `else` (e.g. `if (n != 0) { ...; br 1 }` with no else-arm, exactly the shape `assert_native_exec_loop_counter`'s WASM uses), `else_lbl` was never bound to any address at all, so the false-branch `bcond_label(EQ, else_lbl)` resolved against an unresolved label. This is independent of regalloc (x86-64's own naive.rs doesn't have the analogous bug — confirmed by `test_native_x86_64_sysv_exec_loop_counter` passing throughout) and was very likely already broken before this session's regalloc work started; it just happened not to be caught by `test_native_aarch64_sysv_exec_loop_counter` failing loudly until this integration's code-layout changes shifted the unresolved label's accidental resolution away from whatever address it used to land on. Fixed in `naive.rs`'s `Instruction::End` handler: for `Endable::If`, if `else_lbl` isn't the `usize::MAX` sentinel `Else`'s handler leaves behind when it *did* run, bind it here too, coinciding with `end_lbl` (an else-less `if`'s false branch should land exactly where execution would fall through after the `end` anyway).
   - Verified: `cargo test -p portal-solutions-blitz-tests` → 361/361, matching this doc's own bar.
