...
8. **Give x86-64 a regalloc-backed data-flow core, reached through `sysv.rs`** — bigger and more delicate than it first looks; read `crates/blitz-x86-64/src/sysv.rs` in full before touching it. Two findings that reshape this step from "add a new fallback module" into "also modify sysv.rs's own control-flow arms":
   - **Control flow is already fine.** `sysv.rs`'s own `SysVCtrl` enum (`Loop(top)`/`Block(exit)`/`If{base,else_seen}`) already implements the compile-time label-stack model independently of `naive.rs` (own `ctrl_stack`, no CTX), and its shape lines up almost exactly with `control_flow::Frame` (`base`/`base+1`/`base+2` ≡ `then`/`else_`/`end`). It is already tested and working — there is no forced-migration need here, and swapping `SysVCtrl` for `Frame` outright is a nicety, not a requirement (verify `set_label`-called-twice tolerance on x86-64's iced-based `Writer` before doing so — `close_frame`'s unconditional re-placement of `else_` relies on that, confirmed safe for RISC-V but unverified for x86-64; `SysVCtrl`'s explicit `else_seen` guard suggests the original author wasn't certain either).
   - **The real gap, and the risky part: `SysVState` has no regalloc field, and `sysv.rs`'s own control-flow arms (`Loop`/`Block`/`If`/`Else`/`End`/`Br`/`BrIf`/`BrTable`/`Return`/`sysv_emit_epilogue`) read/write the WASM operand stack *directly through raw `pop`/`push` at `[RSP]`*, exactly like `naive.rs`. A regalloc core can't just be a new delegate module sitting next to that — every one of those arms must flush the regalloc to the real RSP-based stack *before* its existing raw-stack logic runs (mirroring `emit_control_flow_probe`/`br`/etc.'s `ralloc.flush()` calls in `blitz-riscv64::naive`), or a register-held operand will silently read as garbage the moment SysV's own control flow touches `[RSP]`. Concretely: add `regalloc: Option<RegAlloc<x86_regalloc::RegKind, N, Frames<..>>>` to `SysVState`, add a `flush()` call at the top of each of those arms (using the new x86-64 `RegAllocW`'s `ControlFlowWriter::flush`), *then* change the `other => { ... }` fallback (currently delegating to `naive::_handle_op`) to delegate to a new regalloc-based core instead.
   - New module (e.g. `blitz-x86-64/src/regalloc_core.rs`, mirroring `blitz-riscv64::naive`'s shape) implementing const/arithmetic/comparison/local/load-store via `regalloc_frontend`, with instruction selection informed by (not copied verbatim from, since x86-64's regalloc may free different scratch registers) `naive.rs`'s existing per-op mnemonic choices (e.g. the `lea`+truncate trick for `I32Add`/`I64Add`, `not`+`lea` for `Sub`). Needs an x86-64 `RegAllocW` adapter in `codegen.rs` mirroring RISC-V's.
   - Verify against the existing `assert_native_sysv_const`-style real-execution tests (regression) plus new ones for the newly-regalloc-backed ops (this is genuinely new, previously-untested behavior on the SysV path, unlike RISC-V's steps 3/4/6 which were pure extraction of already-tested logic — treat this increment's own new tests as load-bearing, not optional).
   Not started.
