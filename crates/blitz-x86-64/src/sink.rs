    ...
    HandleOpError<Infallible>: From<W::Error>,
{
    fn push_ambient_addr(
        ...
        ctx.writer
            .lea_label(
                ...
                self.0.arch,
                &RAX,
                X64Label::Ambient { name: name.into() },
            )
            .map_err(HandleOpError::from)?;
        ctx.writer
            .push(&mut ctx.asm_ctx, self.0.arch, &RAX)
            .map_err(HandleOpError::from)
    }
    fn call_ambient(
        &mut self,
        ...
        for i in (0..n_params.min(6)).rev() {
            ctx.writer
                .pop(&mut ctx.asm_ctx, self.0.arch, &X64_ARG_REGS[i])
                .map_err(HandleOpError::from)?;
        }
        ctx.writer
            .lea_label(
            ...
            .map_err(HandleOpError::from)?;
        ctx.writer
            .call(&mut ctx.asm_ctx, self.0.arch, &RAX)
            .map_err(HandleOpError::from)?;
        if n_results > 1 {
            ctx.writer
                .push(&mut ctx.asm_ctx, self.0.arch, &RDX_REG)
                .map_err(HandleOpError::from)?;
        }
        if n_results > 0 {
            ctx.writer
    ...
    fn jump_ambient(
        ...
        ctx.writer
            ...
            .map_err(HandleOpError::from)?;
        ctx.writer
            .jmp(&mut ctx.asm_ctx, self.0.arch, &RAX)
            .map_err(HandleOpError::from)
    }
}
