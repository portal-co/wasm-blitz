...
/// Feature-gated trace macro.  Enable with `--features portal-solutions-blitz-riscv32/log`.
#[cfg(feature = "log")]
macro_rules! trace {
    ($($arg:tt)*) => { eprintln!("[riscv32-naive] {}", format_args!($($arg)*)) };
}
#[cfg(not(feature = "log"))]
macro_rules! trace {
    ($($arg:tt)*) => {};
...
use portal_solutions_asm_riscv32::ConditionCode;
...
use portal_solutions_asm_riscv32::RegisterClass;
use portal_solutions_asm_riscv32::out::arg::{ArgKind, MemArgKind};

use portal_solutions_asm_regalloc as regalloc;
use portal_solutions_asm_riscv32 as asm_riscv;
use portal_solutions_asm_riscv32::regalloc as riscv_regalloc;

/// Static Context Register (SCR) — S10 (x26) on RISC-V 32 (ILP32).
///
...
/// Sharding state for RISC-V 32 functions — same design as x86-64/AArch64.
#[derive(Clone, Copy)]
...
pub trait WriterExt<Context>: Writer<RiscvLabel, Context> {
    ...
    fn emit_control_flow_probe(&mut self, ctx: &mut Context, arch: RiscV32Arch, state: &mut State<'_>)
        -> Result<(), Self::Error>
    where
        Self: Sized,
    {
        ...
        Self: Sized,
    {
        ...
        // On RV32 ILP32, WASM addresses already live in 32-bit GPRs — no zext.
        ...
        // Host pointer load: 4-byte ILP32.
        self.lw(ctx, arch, &scratch, &MemArgKind::Mem {
            base: ArgKind::Reg { reg: scratch, size: MemorySize::_32 },
            offset: None, disp: 0, size: MemorySize::_32, reg_class: RegisterClass::Gpr,
        })?;
        ...
        Self: Sized,
    {
        ...
        match op {
            ...
            // I32Add is the same as I64Add at the regalloc/register level — on RV32,
            // GPRs are 32-bit so i32 arithmetic is native; i64 soft-expands elsewhere.
            Instruction::I32Add |
            Instruction::I64Add => {
                let mut rw = crate::codegen::RegAllocW { writer: self, ctx, arch, regalloc: &mut state.regalloc };
            ...
            Instruction::Call(function_index) => {
                ...
                match target {
                    Some(CallTarget::CrossShard { table_slot }) => {
                        // Cross-shard: load fn ptr from [SCR + table_slot * 4] into t0 (ILP32).
                        let t0 = Reg(5);
                        let mem = MemArgKind::Mem {
                            base: ArgKind::Reg { reg: SCR, size: MemorySize::_32 },
                            offset: None,
                            disp: table_slot as i32 * 4,
                            size: MemorySize::_32,
                            reg_class: RegisterClass::Gpr,
                        };
                        self.lw(ctx, arch, &t0, &mem)?;
            ...
            Instruction::MemorySize(_) => {
                ...
                // Dereference: load 32-bit page count.
                let mem = MemArgKind::Mem {
                    base: ArgKind::Reg { reg: dest, size: MemorySize::_32 },
                    offset: None,
                    disp: 0,
                    size: MemorySize::_32,
                    reg_class: RegisterClass::Gpr,
