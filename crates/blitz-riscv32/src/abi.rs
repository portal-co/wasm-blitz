//! [`BackendAbi`] implementations for RISC-V 32.
//!
//! Provides `impl BackendAbi<W, Context> for NaiveAbi` (blitz WASM ABI) and
//! `impl BackendAbi<W, Context> for SysVAbi` (RISC-V psABI ILP32).

extern crate alloc;

use portal_solutions_asm_riscv32::{RiscV32Arch, RegisterClass, out::arg::{ArgKind, MemArgKind}};
...
/// Blitz WASM calling convention for RISC-V 32.
pub struct NaiveAbi;
/// RISC-V psABI ILP32 calling convention.
pub struct SysVAbi;
...
// ---------------------------------------------------------------------------
// NaiveAbi — blitz WASM ABI for RISC-V 32
//
    ...
    W: NaiveWriterExt<Context>,
{
    ...
    fn emit_throw(_w: &mut W, _ctx: &mut Context, _arch: RiscV32Arch, _state: &mut NaiveState, _tag_index: u32, _arity: u32) -> Result<(), W::Error> {
        todo!("emit_throw via BackendAbi — use handle_op_ directly")
    }
    fn emit_try_table_start(_w: &mut W, _ctx: &mut Context, _arch: RiscV32Arch, _state: &mut NaiveState, _catches: &[Catch], _sigs: &[FuncType], _tags: &[u32]) -> Result<(), W::Error> {
        todo!("emit_try_table_start via BackendAbi — use handle_op_ directly")
    }
    fn emit_try_table_end(_w: &mut W, _ctx: &mut Context, _arch: RiscV32Arch, _state: &mut NaiveState, _catches: &[Catch], _sigs: &[FuncType], _tags: &[u32]) -> Result<(), W::Error> {
        todo!("emit_try_table_end via BackendAbi — use handle_op_ directly")
    }
}

// ---------------------------------------------------------------------------
// SysVAbi — RISC-V psABI ILP32 calling convention
// ---------------------------------------------------------------------------
