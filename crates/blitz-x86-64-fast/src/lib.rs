...
pub mod fast {
    ...
    use portal_solutions_asm_x86_64 as asm_x86;
    ...
    impl From<FastError> for alloc::string::String {
        fn from(e: FastError) -> Self { format!("{:?}", e) }
    }
    ...
    fn emit_cmds<E: core::error::Error, W: asm_x86::out::Writer<X64FastLabel, Error = E>>(
        ...
        while let Some(cmd) = it.next() {
            x86_regalloc::process_cmd(writer, arch, &cmd)?;
        }
        Ok(())
    }
    ...
    pub trait WriterExt: asm_x86::out::Writer<X64FastLabel> {
        fn br(
            ...
            let rsp_mem = asm_x86::out::arg::MemArgKind::Mem { base: RSP, offset: None, disp: 8u32, size: MemorySize::_64, reg_class: asm_x86::RegisterClass::Gpr };
        ...
        fn handle_op(
        ...
        ) -> Result<(), Self::Error> where Self: Sized, Self::Error: From<core::fmt::Error> {
            ...
            match op {
                Instruction::I32Const(value) => {
                    {
                        let (r, cmds) = state.regalloc.as_mut().unwrap().push(x86_regalloc::RegKind::Int).map_err(|_| {
                            // convert error
                            core::fmt::Error
                        })?;
                ...
                Instruction::I64Const(value) => {
                    {
                        let (r, cmds) = state.regalloc.as_mut().unwrap().push(x86_regalloc::RegKind::Int).map_err(|_| core::fmt::Error)?;
                        emit_cmds(self, arch, cmds)?;
                        let phys = Reg(r as u8);
                ...
                Instruction::F32Const(value) => {
                    {
                        let (r, cmds) = state.regalloc.as_mut().unwrap().push(x86_regalloc::RegKind::Float).map_err(|_| core::fmt::Error)?;
                        emit_cmds(self, arch, cmds)?;
                        let phys = Reg(r as u8);
                ...
                Instruction::I32Add | Instruction::I64Add => {
                    ...
                    let t2;
                    {
                        let (tt1, cmds1) = state.regalloc.as_mut().unwrap().pop(x86_regalloc::RegKind::Int);
                        emit_cmds(self, arch, cmds1)?;
                        t1 = tt1;
                    }
                    {
                        let (tt2, cmds2) = state.regalloc.as_mut().unwrap().pop(x86_regalloc::RegKind::Int);
                        emit_cmds(self, arch, cmds2)?;
                        t2 = tt2;
                    }
                    let r1 = Reg(t1.reg);
                    let r2 = Reg(t2.reg);
                    self.lea(arch, &r1, &asm_x86::out::arg::MemArgKind::Mem { base: r1, offset: Some((r2, 0)), disp: 0, size: MemorySize::_64, reg_class: asm_x86::RegisterClass::Gpr })?;
                    // push existing
                    {
                        let iter = state.regalloc.as_mut().unwrap().push_existing(regalloc::Target { reg: t1.reg, kind: t1.kind });
                        emit_cmds(self, arch, iter)?;
                    }
                }
                Instruction::LocalGet(local_index) => {
                    // Use regalloc to push local as stack value
                    {
                        let cmds = state.regalloc.as_mut().unwrap().push_local(x86_regalloc::RegKind::Int, *local_index).map_err(|_| core::fmt::Error)?;
                        emit_cmds(self, arch, cmds)?;
                    }
                }
                Instruction::LocalSet(local_index) => {
                    ...
                    let t;
                    {
                        let (tt, cmds) = state.regalloc.as_mut().unwrap().pop(x86_regalloc::RegKind::Int);
                        emit_cmds(self, arch, cmds)?;
                        t = tt;
                    }
                    {
                        let it = state.regalloc.as_mut().unwrap().pop_local(x86_regalloc::RegKind::Int, *local_index);
                ...
                Instruction::I64Load(memarg) => {
                    ...
                    self.lea(arch, &addr, &asm_x86::out::arg::MemArgKind::Mem { base: addr, offset: Some((imm,0)), disp: 0, size: MemorySize::_64, reg_class: asm_x86::RegisterClass::Gpr })?;
                    self.mov(arch, &addr, &addr)?;
                    // push existing
                    {
                        let mut it = state.regalloc.as_mut().unwrap().push_existing(regalloc::Target { reg: t.reg, kind: t.kind });
                        emit_cmds(self, arch, it)?;
                    }
                }
                Instruction::I64Store(memarg) => {
                    ...
                    let addr;
                    {
                        let (vv, cmds_val) = state.regalloc.as_mut().unwrap().pop(x86_regalloc::RegKind::Int);
                        emit_cmds(self, arch, cmds_val)?;
                        val = vv;
                    }
                    {
                        let (aa, cmds_addr) = state.regalloc.as_mut().unwrap().pop(x86_regalloc::RegKind::Int);
                        emit_cmds(self, arch, cmds_addr)?;
                        addr = aa;
                    }
                    let imm = Reg(0);
                    self.mov64(arch, &imm, memarg.offset)?;
                    let base = Reg(addr.reg);
                    self.lea(arch, &base, &asm_x86::out::arg::MemArgKind::Mem { base, offset: Some((imm, 0)), disp: 0, size: MemorySize::_64, reg_class: asm_x86::RegisterClass::Gpr })?;
                ...
                Instruction::BrTable(targets, default) => {
                    for relative_depth in targets.iter().cloned() {
                        ...
                        self.set_label(arch, X64FastLabel::Indexed { idx: i })?;
                        let mut tmp = Reg(0);
                        self.lea(arch, &tmp, &asm_x86::out::arg::MemArgKind::Mem { base: tmp, offset: None, disp: 0xffff_ffff, size: MemorySize::_64, reg_class: asm_x86::RegisterClass::Gpr })?;
                        {
                            emit_cmds(self, arch, state.regalloc.as_mut().unwrap().push_existing(regalloc::Target { reg: tmp.0, kind: x86_regalloc::RegKind::Int }))?;
                        }
                    }
                    let t;
                    {
                    ...
                    {
                        emit_cmds(self, arch, state.regalloc.as_mut().unwrap().push_existing(regalloc::Target { reg: t.reg, kind: t.kind }))?;
                    }
                    {
                        let flush = state.regalloc.as_mut().unwrap().flush();
                        emit_cmds(self, arch, flush)?;
                ...
                Instruction::Else => {
                    let Endable::If { idx: i } = state.if_stack.last().unwrap() else { todo!() };
                ...
                Instruction::Call(function_index) => {
                    ...
                    if let Some(("blitz", h)) = func_imports.get(*function_index as usize) {
                        if h.starts_with("hypercall") {
                            ...
                            self.lea_label(arch, &Reg(0), X64FastLabel::Func { r#fn: function_index })?;
                ...
                Instruction::Return => {
                    ...
                    self.mov(arch, &Reg(0), &Reg::CTX)?;
                    let mut tmp = Reg(0);
                    self.lea(arch, &tmp, &asm_x86::out::arg::MemArgKind::Mem { base: tmp, offset: None, disp: (state.local_count + 3 * 8) as u32, size: MemorySize::_8, reg_class: asm_x86::RegisterClass::Gpr })?;
